...
export default async function BiometricConsentPage({ searchParams }: { searchParams: Promise<{ userId?: string }> }) {
  const sesion = await exigirSesion();
  const { userId: userIdParam } = await searchParams;
  const userId = userIdParam || sesion.userId;

  const consent = await api.get<BiometricConsent | null>(`/attendance-realtime/biometric-consent/${userId}`);

  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-lg font-semibold text-neutral-900">Consentimiento biométrico (UC-ATT-05)</h2>
        <p className="text-sm text-neutral-500">Independiente del consentimiento general de cuenta — específico y revocable en cualquier momento.</p>
      </div>
